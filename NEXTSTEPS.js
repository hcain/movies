// NEXT STEPS:
  ...
  // X  -> have index.js call db
  // _  -> compare times to see if movie has passed


  // _ convert table.tsx to typescript
  // X -> convert basic table to typescript
  // X -> add grayed out time if start time has passed
  // X -> add sort movie titles by alphanumeric
  // X -> add sort time by dateTime
  // _ -> filter drop down for theatre and neighborhood 

  
  // _ set up cron job for 12-hourly scrape
  // X  -> move scrape functions to api point
  // X  -> connect data to hosted db
  // _  -> convert basic table to typescript
  // _  -> remove all runtime errors
  // _  -> deploy to vercel
  ...
  // CLEAN UP TODOs:
  // X convert dateFunctions.tsx to proper typescript
  // X refactor dateFunctions function (and then validMovieTime) to remove format
  // _ add sorting indicatior for time that does not have the a/z on it
  ...
  // H look up proper typescript for recursion (scrapeGoogle function)